...
export default function BookReaderPage() {
  ...
  useEffect(() => {
    ...
    const syncPageHeights = () => {
      ...
      requestAnimationFrame(() => {
        ...
        // Keep increasing maxPageHeight - never decrease within same font size
        setMaxPageHeight(prevHeight => Math.max(prevHeight, maxHeight));

        // Third pass: apply the max height to all pages immediately
        allPageElements.forEach(pageEl => {
          pageEl.style.height = `${Math.max(maxPageHeight, maxHeight)}px`;
        });
      });
    };
  ...
  }, [fontSize, currentPageIndex, bookOpened, maxPageHeight]);
